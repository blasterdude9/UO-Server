...
//Updated gump so we can craft Ignots: "24/04/2013" - Giel
//Updated Corrected recourse label: "24/04/2013" - Giel
//Updated Font: "29/05/20123" - Giel
namespace Server.Engines.Craft
{
	public class CraftGump : Gump
	{
		...
		private CraftGump( Mobile from, CraftSystem craftSystem, BaseTool tool, object notice, CraftPage page ) : base( 40, 40 )
		{
			...
			AddImageTiled( 10, 312, 150, 45, 2624 );
			AddImageTiled( 165, 312, 355, 45, 2624 );
			AddImageTiled( 10, 362, 510, 85, 2624 );
			AddImageTiled( 10, 37, 200, 270, 2624 );
			AddImageTiled( 215, 37, 305, 270, 2624 );
			...
			AddHtmlLocalized( 10, 327, 150, 25, 1044012, LabelColor, false, false ); // <CENTER>NOTICES</CENTER>

			AddButton( 15, 422, 4017, 4019, 0, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 50, 425, 150, 18, 1011441, LabelColor, false, false ); // EXIT

			AddButton( 270, 422, 4005, 4007, GetButtonID( 6, 2 ), GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 305, 425, 150, 18, 1044013, LabelColor, false, false ); // MAKE LAST

			// Mark option
			if ( craftSystem.MarkOption )
			{
				AddButton( 270, 382, 4005, 4007, GetButtonID( 6, 6 ), GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 385, 150, 18, 1044017 + (context == null ? 0 : (int)context.MarkOption), LabelColor, false, false ); // MARK ITEM
			}
			// ****************************************
			...
			if ( craftSystem.Resmelt )
			{
				...
				AddHtmlLocalized( 50, 365, 150, 18, 1044259, LabelColor, false, false ); // SMELT ITEM
			}
			// ****************************************
			...
			{
				AddButton( 270, 362, 4005, 4007, GetButtonID( 6, 5 ), GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 365, 150, 18, 1044260, LabelColor, false, false ); // REPAIR ITEM
			}
			// ****************************************
			...
			if ( craftSystem.CanEnhance )
			{
				...
				AddHtmlLocalized( 305, 4005, 150, 18, 1061001, LabelColor, false, false ); // ENHANCE ITEM
			}
			// ****************************************

			if ( notice is int && (int)notice > 0 )
				AddHtmlLocalized( 170, 327, 350, 40, (int)notice, LabelColor, false, false );
			...
			if ( craftSystem.CraftSubRes.Init )
			{
				...
				if ( nameNumber > 0 )
                    ...
                    AddHtml( 50, 385, 250, 18, String.Format( "<BASEFONT COLOR=#{0:X6}>{1} ({2})</BASEFONT>", FontColor, nameString, resourceCount ), false, false );
			}
			// ****************************************
		...
		public void CreateResList( bool opt, Mobile from )
		{
			...
			for ( int i = 0; i < res.Count; ++i )
			{
				...
				if ( subResource.NameNumber > 0 )
					...
					AddHtml( 255, 63 + (index * 20), 250, 18, String.Format( "<BASEFONT COLOR=#{0:X6}>{1} ({2})</BASEFONT>", FontColor, subResource.NameString, resourceCount ), false, false );
			}
		}
		...
		public int CreateGroupList()
		{
			...
			for ( int i = 0; i < craftGroupCol.Count; i++ )
			{
				...
				if ( craftGroup.NameNumber > 0 )
					...
					AddHtml( 50, 83 + (i * 20), 150, 18, String.Format("<BASEFONT COLOR=#{0:X6}>{1}</BASEFONT>", FontColor, craftGroup.NameString), false, false );
			}
